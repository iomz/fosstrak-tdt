...
public class TestCaseSgtin extends TestCase {
	...
	public void testPage13Staged() {
		...
		// jpb trying this version using a staged approach, only going one level
		// at a time.
		...
		String s2 = engine.convert(s, params, LevelTypeList.TAG_ENCODING);
		...
		String s3 = engine.convert(s2, params, LevelTypeList.PURE_IDENTITY);
		...
		String s4 = engine.convert(s3, params, LevelTypeList.LEGACY);
	...
	public void testPage13() {
		...
		String s2 = engine.convert(s, params, LevelTypeList.LEGACY);
	...
	public void testPage26() {
		...
		params.put("filter", "3");
		params.put("gs1companyprefixlength", "7");
		String legacy[] = { "gtin=00037000302414;serial=10419703",
				"sscc=000370003024147856", "gln=0003700030247;serial=1041970",
		...
		for (String s : legacy) {
			String s2 = engine.convert(s, params, LevelTypeList.BINARY);
			System.out.println("  " + s + " -> " + s2);
			String s3 = engine.convert(s2, params, LevelTypeList.LEGACY);
	...
	public void testCSVTestSet() throws ParseException, IOException,
			ProcessingException {
		...
		if (file.exists()) {
			...
			fp.processFile(testFile, new CSVFieldMapProcessor() {
				...
				public void processDataLine(final int linenumber,
					...
					// "Line #%d: field: %s=%s", linenumber, field.getKey(),
					// field.getValue() ));
	 ...
	 * 
	 * URL auxurl = file.toURL(); System.err.println(auxurl); URL schemeurl =
	 * new URL(
	 * "file:\\\\C:\\Documents and Settings\\floerkem\\Desktop\\tdttrunk\\target\\classes\\schemes"
